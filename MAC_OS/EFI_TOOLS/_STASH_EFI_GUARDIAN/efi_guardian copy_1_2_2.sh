...
# Autore: fgirolami29,Nolan | Versione: 1.2.2 | Edited: 10-09-2025
set -euo pipefail
...
detect_efi() {
    # default: prima EFI interna
    local first
    first="$(diskutil list internal | awk '/EFI/ && $NF ~ /disk[0-9]+s[0-9]+/ {print $NF; exit}')"
    EFI_ID="${first:-}"
    if [[ -z "$EFI_ID" ]]; then
        echo -e "${YLW}Nessuna EFI interna trovata. Mostro tutte le EFI…${NC}"
        mapfile -t ALL < <(diskutil list | awk '/EFI/ {print $NF}')
        ((${#ALL[@]})) || {
            echo -e "${RED}Nessuna partizione EFI trovata.${NC}"
            return 1
        }
        echo "EFI disponibili:"
        local i=1
        for p in "${ALL[@]}"; do
            echo "  $i) $p"
            ((i++))
        done
        read -rp "Scegli ID (numero): " n
        ((n >= 1 && n <= ${#ALL[@]})) || {
            echo -e "${RED}Selezione invalida.${NC}"
            return 1
        }
        EFI_ID="${ALL[$((n - 1))]}"
    fi
...
pick_external_disk() {
    ...
    ((n >= 1 && n <= ${#DISKS[@]})) || {
        echo "Selezione invalida"
        return 1
    }
    echo "${DISKS[$((n - 1))]#/dev/}" # stampa “diskX”
}
...
task_restore_zip() {
    ...
    read -rp "Confermi? (yes in minuscolo): " ok
    [[ "$ok" == "yes" ]] || {
        echo "Annullato."
        return 1
    }
...
task_restore_raw() {
    #    ensure_sudo
    read -rp "Percorso immagine RAW (.img): " img
    [[ -f "$img" ]] || {
        echo -e "${RED}File non trovato.${NC}"
        return 1
    }
    read -rp "EFI di destinazione (es. disk0s1): " target
    [[ -n "$target" ]] || {
    ...
    read -rp "Digita esattamente: I UNDERSTAND  " conf
    [[ "$conf" == "I UNDERSTAND" ]] || {
        echo "Annullato."
        return 1
    }
